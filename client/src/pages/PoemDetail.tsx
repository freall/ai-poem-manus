...
export default function PoemDetail() {
  ...
  // 获取诗词详情
  const { data: poemData } = trpc.poems.getById.useQuery({ id: poemId });
  const poem = poemData?.poem;
  const questions = poemData?.questions || [];
  ...
  const handleAnswerClick = (answer: string) => {
    ...
    if (answer === currentQuestion?.correctAnswer) {
      ...
      setTimeout(() => setCelebrateShow(false), 2000);
    }
  };
            ...
            <Card className="mb-8 border-2 border-purple-200 bg-white/95 backdrop-blur shadow-xl">
              ...
              <CardContent className="pt-8">
                ...
                <div className="mb-8 p-6 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-lg border-2 border-yellow-200">
                  <p className="text-2xl leading-relaxed text-gray-800 font-serif whitespace-pre-line">
                    {poem.content}
                  </p>
                </div>
            ...
            <Card className="sticky top-24 border-2 border-blue-200 bg-white/95 backdrop-blur shadow-xl">
              <CardHeader className="bg-gradient-to-r from-blue-100 to-cyan-100">
                <CardTitle className="text-2xl">
                  📝 问答 {currentQuestionIndex + 1}/{questions.length}
                </CardTitle>
              </CardHeader>
